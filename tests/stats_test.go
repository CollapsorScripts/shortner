...
func Test_GetStats_Good(t *testing.T) {
	_, st := suite.New(t)

	endpoint := fmt.Sprintf("%s%s", st.BaseURL, "stats/1BbKte")
	...
	if err := json.Unmarshal(respBytes, &statsResponse); err != nil {
		t.Errorf("Ошибка при анмаршлинге тела ответа: %v; body=%q", err, string(respBytes))
		return
	}

	t.Logf("Ответ: %s", utils.ToJSON(statsResponse))
}
